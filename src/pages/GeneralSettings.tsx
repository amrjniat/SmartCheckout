...
import {
  Building2, Settings, Receipt, Coins, Package, Languages,
  Save, Check, Search, Upload, Info, RotateCcw, X, AlertCircle
} from 'lucide-react';
...
const translations = {
  ar: {
    ...
    logoTooLarge: 'حجم الملف كبير جداً (الحد الأقصى 2 ميجابايت)',
  },
  en: {
    pageTitle: 'General Settings',
    pageSubtitle: 'Manage all SmartCheckout settings',
    ...
    logoTooLarge: 'File is too large (max 2MB)',
  },
};

// ================= Option maps (UI label <-> backend value) =================
const fiscalYearOptions = (t: typeof translations['ar']) => [
  { value: 1, label: t.fiscalJan },
  { value: 4, label: t.fiscalApr },
  { value: 7, label: t.fiscalJul },
];

const currencyOptions = (t: typeof translations['ar']) => [
  { value: 'SYP', label: t.currencySyp },
  { value: 'USD', label: t.currencyUsd },
];

const symbolPositionOptions = (t: typeof translations['ar']) => [
  { value: 'AfterNumber', label: t.symbolAfter },
  { value: 'BeforeNumber', label: t.symbolBefore },
];

const taxCalcOptions = (t: typeof translations['ar']) => [
  { value: 'AfterDiscount', label: t.taxAfterDiscount },
  { value: 'BeforeDiscount', label: t.taxBeforeDiscount },
];

const languageOptions = (t: typeof translations['ar']) => [
  { value: 'ar', label: t.optArabic },
  { value: 'en', label: t.optEnglish },
];

const directionOptions = (t: typeof translations['ar']) => [
...
export default function GeneralSettings() {
  ...
  const [systemForm, setSystemForm] = useState<SystemSettingsData>({});
  const [companyForm, setCompanyForm] = useState<CompanyInfoData>({});

  const fileInputRef = useRef<HTMLInputElement>(null);

  // ================= تحميل البيانات الحقيقية عند فتح الصفحة =================
  useEffect(() => {
    let cancelled = false;
  ...
  const updateSystem = <K extends keyof SystemSettingsData>(field: K, value: SystemSettingsData[K]) => {
    setSystemForm(prev => ({ ...prev, [field]: value }));
    setHasUnsavedChanges(true);
    setIsSaved(false);
  };

  const updateCompany = <K extends keyof CompanyInfoData>(field: K, value: CompanyInfoData[K]) => {
    setCompanyForm(prev => ({ ...prev, [field]: value }));
    setHasUnsavedChanges(true);
    setIsSaved(false);
  };

  // Checkbox واحد بالفرونت بيتحكم بحقلين بالباك (بيع + استرجاع)
  const updateAutoUpdateStock = (value: boolean) => {
    setSystemForm(prev => ({
      ...prev,
      autoUpdateStockAfterSale: value,
      autoUpdateStockAfterReturn: value,
    }));
    setHasUnsavedChanges(true);
    setIsSaved(false);
  };
  ...
  const handleDiscardChanges = async () => {
    ...
    try {
      ...
      console.error('❌ فشل التراجع عن التعديلات:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLogoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCompanyForm(prev => ({ ...prev, logoPath: res.logoPath }));
    } catch (err) {
      console.error('❌ فشل رفع الشعار:', err);
      setSaveErr(t.logoUploadError);
    } finally {
  ...
  const handleLogoRemove = async () => {
    ...
    try {
      await deleteLogo();
      setCompanyForm(prev => ({ ...prev, logoPath: null }));
    } catch (err) {
      console.error('❌ فشل حذف الشعار:', err);
      setSaveErr(t.logoUploadError);
    } finally {
      setIsUploadingLogo(false);
    }
  ...
  const logoUrl = getLogoUrl(companyForm.logoPath);

  if (isLoading) {
    return (
      <div dir={isRtl ? 'rtl' : 'ltr'} className="flex min-h-dvh items-center justify-center bg-gradient-to-b from-[#F8FAFC] to-white">
    ...
    return (
      <div dir={isRtl ? 'rtl' : 'ltr'} className="flex min-h-dvh items-center justify-center bg-gradient-to-b from-[#F8FAFC] to-white">
        <div className="flex flex-col items-center gap-3 text-center max-w-sm">
          ...
          <p className="text-sm text-gray-700">{loadErr}</p>
          <button
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="flex min-h-dvh flex-col lg:flex-row bg-gradient-to-b from-[#F8FAFC] to-white text-gray-800 font-sans overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col relative min-h-dvh overflow-hidden">
        ...
        {saveErr && (
          <div className="mx-4 sm:mx-8 mt-4 flex items-center justify-between gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-2.5 rounded-lg">
            <span className="flex items-center gap-2"><AlertCircle size={16} /> {saveErr}</span>
            <button onClick={() => setSaveErr(null)}><X size={16} /></button>
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            {activeTab === 'company' && (
              <>
                <SettingsCard title={t.companyLogo}>
                  <div className="flex items-center gap-4">
                    <div
                      ...
                      className="w-16 h-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-500 cursor-pointer hover:bg-gray-100 hover:border-blue-400 transition-all overflow-hidden relative"
                    >
                      {isUploadingLogo ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        {logoUrl && (
                          <button
                            ...
                            className="px-3 py-1.5 text-red-600 bg-red-50 rounded-lg text-xs font-medium hover:bg-red-100 transition-colors disabled:opacity-50"
                          >
                            {t.remove}
                          </button>
                        )}
                ...
                <SettingsCard title={t.companyData}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <InputField label={t.companyName} value={companyForm.companyName || ''} onChange={(e) => updateCompany('companyName', e.target.value)} isRtl={isRtl} />
                    <InputField label={t.branchName} value={companyForm.branchName || ''} onChange={(e) => updateCompany('branchName', e.target.value)} isRtl={isRtl} />
                  </div>
                </SettingsCard>

                <SettingsCard title={t.contactInfo}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <InputField label={t.phone} value={companyForm.phone || ''} dir="ltr" onChange={(e) => updateCompany('phone', e.target.value)} isRtl={isRtl} />
                    <InputField label={t.email} value={companyForm.email || ''} dir="ltr" onChange={(e) => updateCompany('email', e.target.value)} isRtl={isRtl} />
                  </div>
                </SettingsCard>

                <SettingsCard title={t.taxData}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <InputField label={t.taxNumber} value={companyForm.taxNumber || ''} onChange={(e) => updateCompany('taxNumber', e.target.value)} tooltip={t.taxNumberTooltip} isRtl={isRtl} />
                    <InputField label={t.commercialRegister} value={companyForm.commercialRegister || ''} onChange={(e) => updateCompany('commercialRegister', e.target.value)} isRtl={isRtl} />
                  </div>
                </SettingsCard>

                <SettingsCard title={t.address}>
                  <InputField label={t.fullAddress} value={companyForm.address || ''} onChange={(e) => updateCompany('address', e.target.value)} isRtl={isRtl} />
                </SettingsCard>
              </>
            )}
            ...
            {activeTab === 'system' && (
              <SettingsCard title={t.basicSettings}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <InputField label={t.systemName} value={systemForm.systemName || ''} onChange={(e) => updateSystem('systemName', e.target.value)} isRtl={isRtl} />
                  <SelectField
                    label={t.timezone}
                    options={['Asia/Damascus', 'Asia/Riyadh']}
                    value={systemForm.timeZone || 'Asia/Damascus'}
                    dir="ltr"
                  ...
                  <SelectField
                    ...
                    value={systemForm.dateFormat || 'DD/MM/YYYY'}
                    dir="ltr"
                    onChange={(e) => updateSystem('dateFormat', e.target.value)}
                  />
                  <SelectField
                    label={t.fiscalYearStart}
                    options={fiscalYearOptions(t)}
                    value={String(systemForm.fiscalYearStartMonth ?? 1)}
                    onChange={(e) => updateSystem('fiscalYearStartMonth', Number(e.target.value))}
                  />
                </div>
              </SettingsCard>
            )}
            ...
            {activeTab === 'invoice' && (
              <>
                <SettingsCard title={t.invoiceNumbering}>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <InputField label={t.invoicePrefix} value={systemForm.invoicePrefix || ''} dir="ltr" onChange={(e) => updateSystem('invoicePrefix', e.target.value)} isRtl={isRtl} />
                    <InputField label={t.invoicePadding} value={systemForm.invoicePadding ?? 6} type="number" dir="ltr" onChange={(e) => updateSystem('invoicePadding', Number(e.target.value))} isRtl={isRtl} />
                    <InputField label={t.nextNumber} value="INV-000001" disabled dir="ltr" isRtl={isRtl} />
                  </div>
                </SettingsCard>
                ...
                <SettingsCard title={t.printOptions}>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-2">
                      <InputField label={t.autoPrintCopies} value={systemForm.defaultPrintCopies ?? 1} type="number" onChange={(e) => updateSystem('defaultPrintCopies', Number(e.target.value))} isRtl={isRtl} />
                      <InputField label={t.invoiceFooter} value={systemForm.invoiceFooterMessage || ''} onChange={(e) => updateSystem('invoiceFooterMessage', e.target.value)} isRtl={isRtl} />
                    </div>
                    <CheckboxField label={t.showLogoOnInvoice} checked={!!systemForm.showCompanyLogoOnInvoice} onChange={(e) => updateSystem('showCompanyLogoOnInvoice', e.target.checked)} />
                    <CheckboxField label={t.autoPrintOnPayment} checked={!!systemForm.autoPrintOnPayment} onChange={(e) => updateSystem('autoPrintOnPayment', e.target.checked)} />
                  </div>
                </SettingsCard>
              </>
            )}
            ...
            {activeTab === 'currency' && (
              <>
                <SettingsCard title={t.currencySettings}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <SelectField
                      ...
                      value={companyForm.currencyCode || 'SYP'}
                      onChange={(e) => updateCompany('currencyCode', e.target.value)}
                    />
                    <InputField label={t.currencySymbol} value={companyForm.currencySymbol || ''} onChange={(e) => updateCompany('currencySymbol', e.target.value)} isRtl={isRtl} />
                    <SelectField
                      label={t.decimalPlaces}
                      options={['0', '1', '2', '3']}
                      value={String(systemForm.decimalPlaces ?? 2)}
                      onChange={(e) => updateSystem('decimalPlaces', Number(e.target.value))}
                    />
                    <SelectField
                      label={t.symbolPosition}
                      options={symbolPositionOptions(t)}
                      value={systemForm.currencySymbolPosition || 'AfterNumber'}
                      onChange={(e) => updateSystem('currencySymbolPosition', e.target.value)}
                    />
                  </div>
                </SettingsCard>
                ...
                <SettingsCard title={t.taxes}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                    <InputField label={t.defaultTaxRate} value={systemForm.defaultTaxRate ?? 0} type="number" dir="ltr" onChange={(e) => updateSystem('defaultTaxRate', Number(e.target.value))} isRtl={isRtl} />
                    <SelectField
                      label={t.taxCalcMethod}
                      options={taxCalcOptions(t)}
                      value={systemForm.taxCalculationMethod || 'AfterDiscount'}
                      onChange={(e) => updateSystem('taxCalculationMethod', e.target.value)}
                    />
                  </div>
                  <CheckboxField label={t.roundDecimals} checked={!!systemForm.roundDecimals} onChange={(e) => updateSystem('roundDecimals', e.target.checked)} />
                </SettingsCard>
              </>
            )}
            ...
            {activeTab === 'inventory' && (
              <SettingsCard title={t.stockMovement}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
                  <InputField label={t.defaultAlertThreshold} value={systemForm.defaultLowStockAlert ?? 10} type="number" dir="ltr" onChange={(e) => updateSystem('defaultLowStockAlert', Number(e.target.value))} isRtl={isRtl} />
                </div>
                <div className="space-y-4">
                  <CheckboxField label={t.useBarcode} checked={!!systemForm.useBarcodeScannerInPOS} onChange={(e) => updateSystem('useBarcodeScannerInPOS', e.target.checked)} />
                  <CheckboxField label={t.autoUpdateStock} checked={!!systemForm.autoUpdateStockAfterSale} onChange={(e) => updateAutoUpdateStock(e.target.checked)} />
                  <CheckboxField label={t.lowStockAlerts} checked={!!systemForm.enableLowStockAlerts} onChange={(e) => updateSystem('enableLowStockAlerts', e.target.checked)} />
                  <CheckboxField label={t.allowNegativeStock} checked={!!systemForm.allowNegativeStock} onChange={(e) => updateSystem('allowNegativeStock', e.target.checked)} tooltip={t.allowNegativeStockTooltip} />
                </div>
              </SettingsCard>
            )}
              ...
              <SettingsCard title={t.interfaceLanguageSettings}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                  <SelectField
                    ...
                    value={systemForm.defaultLanguage || 'ar'}
                    onChange={(e) => updateSystem('defaultLanguage', e.target.value)}
                  />
                  <SelectField
                    label={t.defaultDirection}
                    options={directionOptions(t)}
                    value={systemForm.defaultDirection || 'rtl'}
                    dir="ltr"
                    onChange={(e) => updateSystem('defaultDirection', e.target.value)}
                  />
                </div>
              </SettingsCard>
            )}
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 px-8 flex items-center justify-between shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-gray-600"
          >
            <RotateCcw size={18} />
            {t.cancelChanges}
          </button>
...
function SelectField({ label, options, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string, options: SelectOption[] }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-xs font-medium text-gray-700">{label}</label>
      <select
