...
// /* ============================================================
//    POSWAVE — صفحة شاشة البيع السريع (ملف واحد)
//    يحتوي: PosNavbar (الهيدر) + QuickSaleScreen (جسم الصفحة) + QuickSalePage (المُصدَّر الافتراضي)
//    كلاهما يشتركان بنفس حالة اللغة isRtl / setIsRtl.
//    ============================================================ */
...
// function PosNavbar({ isRtl: isRtlProp, setIsRtl: setIsRtlProp }: PosNavbarProps = {}) {
//   // دعم الاستخدام المستقل (بدون Layout) عبر حالة داخلية احتياطية،
//   // مع إعطاء الأولوية لأي isRtl/setIsRtl ممرّرة من DashboardLayout
//   const [internalIsRtl, setInternalIsRtl] = useState(true);
//   const isRtl = isRtlProp !== undefined ? isRtlProp : internalIsRtl;
//   const setIsRtl = setIsRtlProp !== undefined ? setIsRtlProp : setInternalIsRtl;
...
//   const tNav = isRtl ? navTranslations.ar : navTranslations.en;

//   // ✅ نفس عناصر القائمة الموجودة في DashboardLayout، مع تفعيل عنصر شاشة البيع السريع (POS)
//   const menuItems = [
//     { id: 'dash', text: isRtl ? 'الصفحه الرئيسيه' : 'Home page', active: false },
//     { id: 'pos', text: isRtl ? 'شاشة البيع السريع (POS)' : 'Quick Sale Screen (POS)', active: true },
...
//       <header dir={isRtl ? 'rtl' : 'ltr'} className="w-full bg-[#0a1931] text-white shadow-md z-30 px-4 flex-shrink-0 select-none">
...
//             <button onClick={() => setIsRtl(!isRtl)} className="text-[11px] font-bold bg-white/10 hover:bg-white/20 px-2.5 py-1.5 rounded-lg min-w-[65px] text-center">{tNav.langBtn}</button>
//             <button className="p-2 rounded-lg text-rose-400 hover:bg-rose-500/10 transition-colors hidden sm:block" title={tNav.logout}>
//               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
//             </button>
//             <div className="flex items-center gap-2">
//               <div className="hidden md:flex flex-col text-xs font-medium text-start">
//                 <span className="text-slate-200">{tNav.userName}</span>
//                 <span className="text-[10px] text-slate-400 opacity-80">{tNav.userTitle}</span>
//               </div>
//               <div className="w-9 h-9 bg-blue-500/20 text-blue-400 rounded-xl flex items-center justify-center font-bold text-sm border border-blue-500/30">{tNav.avatarChar}</div>
//             </div>
//           </div>
...
// /* ============================================================
//    2) QuickSaleScreen — جسم الصفحة (شاشة البيع)
//    ============================================================ */
...
// function QuickSaleScreen({ isRtl: isRtlProp, setIsRtl: setIsRtlProp }: QuickSaleScreenProps = {}) {
//   const [internalIsRtl, setInternalIsRtl] = useState(true);
//   const isRtl = isRtlProp !== undefined ? isRtlProp : internalIsRtl;
//   const setIsRtl = setIsRtlProp !== undefined ? setIsRtlProp : setInternalIsRtl;
...
//       {/* -------- Body (يُعرض مباشرة تحت هيدر PosNavbar) -------- */}
...
// /* ============================================================
//    3) QuickSalePage — المُصدَّر الافتراضي (الهيدر + جسم الصفحة معاً)
//    ============================================================ */
...
//       <PosNavbar isRtl={isRtl} setIsRtl={setIsRtl} />
//       <div className="flex-1 min-h-0">
//         <QuickSaleScreen isRtl={isRtl} setIsRtl={setIsRtl} />
//       </div>
//     </div>
//   );
// }
...
const T = {
  ar: {
    ...
    save: 'حفظ',
    cancel: 'إلغاء',
    deleteProduct: 'حذف المنتج', // تمت الإضافة
    productDeleted: 'تم حذف المنتج بنجاح', // تمت الإضافة
    holdInvoice: 'تعليق الفاتورة',
    print: 'طباعة',
    refund: 'مرتجع',
    selectCustomer: 'اختيار العميل',
  ...
  en: {
    ...
    save: 'Save',
    cancel: 'Cancel',
    deleteProduct: 'Delete Product', // Added
    productDeleted: 'Product deleted successfully', // Added
    holdInvoice: 'Hold Invoice',
    print: 'Print',
    refund: 'Refund',
    selectCustomer: 'Select Customer',
...
function QuickSaleScreen({ isRtl: isRtlProp, setIsRtl: setIsRtlProp }: QuickSaleScreenProps = {}) {
  ...
  const handleAddProduct = () => {
    ...
    const newProd: Product = {
      id: generatedId,
      ar: newProduct.name,
      en: newProduct.name, // للاختصار يتم اعتماد نفس الاسم
      price: priceNum,
  ...
  // حذف منتج (تمت الإضافة)
  const handleDeleteProduct = useCallback(
    (id: string, e: React.MouseEvent) => {
      e.stopPropagation(); // لمنع تفعيل حدث (إضافة للسلة)
      setProductList((prev) => prev.filter((p) => p.id !== id));
      setCart((prev) => prev.filter((c) => c.id !== id)); // إزالة المنتج من السلة إن وجد
